...
class SecretsTestCase(base.TestCase):
    ...
    def test_huge_bogus_algorithm(self):
        ...
        model = secret_models.SecretModel(**secret_create_defaults_data)
        overrides = {
            'algorithm': 'a' * (self.max_field_size + 1)
        }
        model.override_values(**overrides)
        resp, secret_ref = self.behaviors.create_secret(model)
        self.assertEqual(resp.status_code, 400)
    ...
    def test_bogus_encoding(self):
        ...
        model = secret_models.SecretModel(**secret_create_defaults_data)
        overrides = {
            'payload_content_encoding': 'a' * (self.max_field_size + 1)
        }
        model.override_values(**overrides)
        resp, secret_ref = self.behaviors.create_secret(model)
        self.assertEqual(resp.status_code, 400)
    ...
    def test_content_type(self, payload):
        ...
        resp, secret_ref = self.behaviors.create_secret(model, headers)
    ...
    def test_evil_expiration(self, payload):
        """Creates a secret with various evil expiration times
    ...
    def test_cross_user_read(self):
        """Attempt to create a secret with one admin user, and read it with
        another.

        Should return 200 (for now, 401 in future?)"""

        model = secret_models.SecretModel(**secret_create_defaults_data)
        overrides = {
            'name': 'This is a super secret secret'
        }
        model.override_values(**overrides)
        resp, secret_ref = self.behaviors.create_secret(model)
        self.assertEqual(resp.status_code, 201)
